...
public class LogRegressorsOnlyContinuousFeatures extends Parameters<LogisticCoefficients,LogisticCoefficients>  {
	 ...
	 * @param dep
	 */
	public LogRegressorsOnlyContinuousFeatures(Dependency dep){
		this.dep=dep;
		...
		for(Value v:((RangeDiscrete)dep.getHead().getPredicate().getRange()).getValues()){
			init.put(v, new Regression(dep.getContinuousFeatures()));
		}
	...
	public void addConditionalParameter(Value val, Regression parameters) throws WrongParameterNumber, WrongValueType, WrongValueSpecification {
		if(dep.getContinuousFeatures().size()!=parameters.getWeights().size()){
		...
		for(int i=0;i<dep.getContinuousFeatures().size();i++){
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		LogRegressorsOnlyContinuousFeatures other = (LogRegressorsOnlyContinuousFeatures) obj;
