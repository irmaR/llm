...
public class DTWithFeatureSelectionDynamicDiscretization extends DTDependencySelection { 
	...
	private int discr_level;
	private GetFeatures f;
    private NetworkInfo ntw;

	public DTWithFeatureSelectionDynamicDiscretization(String outputDirectory,SpecialPenalties p,int discr_level, GetFeatures displacementFeatures,NetworkInfo ntw){
		...
		this.f=displacementFeatures;
		this.ntw=ntw;
	}


	@Override
	public LearnedDependency selectBestDependency(Atom a,LearnedDependency learned_dep, List<Feature> fts,QueryMachine query_data_training,QueryMachine query_data_validation, QueryMachine query_data_test,
	...
	List<Feature> getNewFeatures(Atom a,Dependency dep,QueryMachine query_data_training, QueryMachine query_data_test,HashMap<Feature, Value> filter) throws Exception{
		...
		List<Feature> newFeatures=this.f.getListOfFeatures(a, ntw, query_data_training,filter);
		//int[] indices=new int[]{0, 1, 9, 324, 175, 27, 187, 330, 128, 139, 251, 322, 57, 183, 174, 172, 345, 86, 320, 36};
		//System.out.println("NEW FEATURES!!!!");
		//System.out.println("FILTERING BASED ON: "+filter);
	    List<Feature> removeFeatures=new ArrayList<Feature>();
	   // for(int i:indices){
	   // 	removeFeatures.add(newFeatures.get(i));
	   // }
		
		//for(Feature f:removeFeatures){
	...
	List<Feature> preSelectFeatures(Atom a,LearnedDependency learnedDep, List<Feature> fts,QueryMachine query_data_training, QueryMachine query_data_test,HashMap<Feature, Value> filter) throws IOException{
		branchCounter++;
		...
		pars.addAll(Arrays.asList(new String[]{"/usr/bin/python"}));
	...
	private LearnedDependency selectBestDependencyContinuousHeadAtomWithScript(Atom a, LearnedDependency learned_dep, List<Feature> fts,QueryMachine query_data_training,QueryMachine query_data_validation, QueryMachine query_data_test,HashMap<Feature, Value> filter, Penalty pen, double parent_score) throws Exception {
		...
		List<Feature> newFeatures=getNewFeatures(a,learned_dep.getDep(),query_data_training,query_data_test,filter);
		List<Feature> preselectedFeatures=preSelectFeatures(a,learned_dep,newFeatures,query_data_training,query_data_test,filter);
	...
	public Double[] getScoreBranchCV(Atom a,HashMap<Value, Double> testErrorPerBranch, HashMap<Value, Double> normalizedTestErrorPerBranch, HashMap<Value, Double> scorePerBranch, HashMap<Value, Double> normalizedScorePerBranch,HashMap<Value,Double> testScorePerBranch,HashMap<Value,Double> normalizedTestScorePerBranch, HashMap<Value,Parameters> parametersPerBranch, String scriptName,RangeDiscrete range,HashMap<Feature,Value> filter,Feature ft,QueryMachine query_data_training,QueryMachine query_data_validation,QueryMachine query_data_test,Dependency dep_temp) throws IOException{
		...
		trainingData=query_data_training.getQueryResults(dep_temp);
		this.trainingData=trainingData;
		query_data_validation.setUseCache(false);
		validationData=query_data_validation.getQueryResults(dep_temp);
		testData=query_data_test.getQueryResults(dep_temp);
		queryingTime+=System.nanoTime()-initQueryingTime;
		int testDataPoints=0;
