...
public class Learn_by_demonstration_Version2 {
	...
	public static void main(String[] args) throws Exception{
		...
		//int[] indices=new int[]{0,1743,7,1407,1455,10,11,8,1719,1335,1746,1745};
		//int[] indices=new int[]{0,1746,1455};
		/*for(int i:indices){
		...
		QueryData init_training=training_data_machine.getQueryResults(dep);
		dep.getCpd().setParameters(dep.getCpd().getCpdEvaluator().estimateParameters(init_training));
		/*FileWriter fw_training=null;
		FileWriter fw_validation=null;
		try {
			fw_training=new FileWriter(new File("file_training"+new Random().nextInt()+".csv"));
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		try {
			fw_validation=new FileWriter(new File("file_validation"+new Random().nextInt()+".csv"));
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}*/
		/*QueryData init_validation=validation_machine.getQueryResults(dep);
		double error_training=dep.getCpd().getCpdEvaluator().getUnnormalizedError(init_training, dep.getCpd().getParameters(),false);
		double error_validation=dep.getCpd().getCpdEvaluator().getUnnormalizedError(init_validation, dep.getCpd().getParameters(),false);
	    System.out.println(error_training+" "+error_validation);*/
		//str_learner.set_selected_feature_indices(indices);
		
		DecisionTreeLearning dtl=new DecisionTreeLearning(new DTDependencySelectorSplitDiscreteAllFeatures(new String[]{"/usr/bin/python",System.getProperty("user.dir").toString()+"/regression.py"},AlgorithmParameters.output_path));
		StructureLearner str_learner=new StructureLearner(fGen,dtl,ntw,training_data_machine,validation_machine,test_machine);	
		int[] indices=new int[]{0, 905, 689, 735, 761, 21};
		...
		/*for(Atom a:trees.getLearnedDependency().keySet()){
			try
			{
				...
				fw.close();
				double acc_error=learned_tree.getAccumulatedError(learned_tree.getRoot(),0);
