...
public class TransitionDisplacementNetwork {
	public NetworkInfo getNetwork(int subsampling_ratio){
		...
		CategoricalPred displacement_discretized_pred=new CategoricalPred("disp", 1,new String[]{"a","b","c","d","e","f","n"});
		CategoricalPred action_pred=new CategoricalPred("action", 1,new String[]{"p","t","g","b"});
		...
		Atom displacement_discretized=new Atom(displacement_discretized_pred, new Logvar[]{object});


		Atom yaw_next=new Atom(yaw_next_pred, new Logvar[]{object});
		Atom pitch_next=new Atom(pitch_next_pred, new Logvar[]{object});
		Atom roll_next=new Atom(roll_next_pred, new Logvar[]{object});
		return new NetworkInfo(new Atom[]{action,displacement_discretized,displacement,yaw_next,pitch_next,roll_next,arm_x_cur,arm_y_cur,arm_z_cur,arm_roll_cur,arm_pitch_cur,arm_yaw_cur,arm_finger,arm_x_next,arm_y_next,arm_z_next,arm_roll_next,arm_pitch_next,arm_yaw_next,arm_finger_next,x_cur,y_cur,z_cur,x_next,y_next,z_next,yaw_cur,pitch_cur,roll_cur},new Type[]{objects,arms});	   		
	}
}
